...
import { TelegramHandler, TelegramUpdate } from './handlers/telegram-handler';
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === '/webhook' && request.method === 'POST') {
      try {
        ...
        const telegramHandler = new TelegramHandler(db, env.TELEGRAM_BOT_TOKEN);
    ...
    if (url.pathname === '/run-health-check' && request.method === 'POST') {
      try {
        ...
        const notificationService = new NotificationService(env.TELEGRAM_BOT_TOKEN);
        const healthCheckService = new HealthCheckService(db, apiService, notificationService);
